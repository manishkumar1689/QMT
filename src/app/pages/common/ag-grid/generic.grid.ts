...
export class TestGridComponent {
  ...
  @Output() GeneralSearchAction: EventEmitter<any> = new EventEmitter();
  ...
  myGridOptions: GridOptions = {
    ...
    getRowHeight: this.detailRowHeight,
    sideBar: true,
    context: {
          ...
          _header = _.merge(this.createColumn(data[key], isChildGrid),
            {
              filter: 'agTextColumnFilter',
              cellRenderer: function (params) {
                if (params.data['associatedProject'] != null && params.data['associatedProject'] != undefined)
                  return '<span style="margin:6px;font-size:12px"><i title="Click to Edit" class="icon icon-search" data-action-type="generalSearch" style="cursor:pointer;"></i></span>'
              }
          ...
          cellRenderer: function (params) {
            ...
            if (params.data != null) {
              ...
              if (localGeneralSettingWithoutPermission) {
                ...
                //let Role: any = params.data['patrole'];
                //if (ActivityStatus == 'Draft' && (Role == 'Practitioner' || Role == 'Admin')) {
                //  templateCol += '<span style="margin:6px;"><i title="Click to delete" class="icon icon-settings" data-action-type="generalDelete" style="cursor:pointer;"></i></span>';
                //} else {
                //  templateCol += '<span style="margin:6px;"><i title="Can not delete this PAT" class="icon icon-settings" style="opacity:0.5;cursor:auto;" disabled="true"></i></span>';
                //}
              }
              if (localChangePracticener) {
                let Role: any = params.data['patrole'];
  ...
  onRowClicked(e) {
    ...
    switch (actionType) {
      ...
      case "generalSearch":
        return this.GeneralSearchAction.emit(data);
      case "generalSetting":
